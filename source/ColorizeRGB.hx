...
class ColorizeRGB {
	public var shader(default, null):ColorizeRGBShader = new ColorizeRGBShader();
	public var red(default, set):Float = 0;
	public var green(default, set):Float = 0;
	public var blue(default, set):Float = 0;

	private function set_red(value:Float) {
		red = value;
		shader.uTime.value[0] = red;
		return red;
	}

	private function set_green(value:Float) {
		green = value;
		shader.uTime.value[1] = green;
		return green;
	}

	private function set_blue(value:Float) {
		blue = value;
		shader.uTime.value[2] = blue;
		return blue;
	}
...
class ColorizeRGBShader extends FlxShader {
	@:glFragmentSource('
