	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false) {
		...
		if (FileSystem.exists(modsListPath))
		{
			...
			for (i in 0...stuff.length)
			{
				...
				if (splitName[1] == '0')	disabledMods.push(splitName[0]); // Disable mod
		...
		for (i in 0...directories.length) {
			...
			if (FileSystem.exists(directory)) {
				...
				for (daWeek in listOfWeeks) {
					...
					if (sys.FileSystem.exists(path)) addWeek(daWeek, path, directories[i], i, originalLength);
				}

				for (file in FileSystem.readDirectory(directory)) {
					var path = haxe.io.Path.join([directory, file]);
					if (!sys.FileSystem.isDirectory(path) && file.endsWith('.json')) addWeek(file.substr(0, file.length - 5), path, directories[i], i, originalLength);
				}
			}
		}
		#end
