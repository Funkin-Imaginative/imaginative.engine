...
class TitleState extends MusicBeatState {
	...
	override public function create():Void {
		...
		if (ClientPrefs.data.checkForUpdates && !closedState) {
			...
			var http = new haxe.Http("https://raw.githubusercontent.com/RodneyAnImaginativePerson/FNF-Imaginative-Engine/main/gitVersion.txt");
	...
	function startIntro() {
		...
		if (animFrames.length > 0) {
			...
			titleText.animation.addByPrefix('press', ClientPrefs.data.flashingLights ? "ENTER PRESSED" : "ENTER FREEZE", 24);
		} else {
			newTitle = false;
	...
	override function update(elapsed:Float) {
		...
		if (initialized && !transitioning && skippedIntro) {
			...
			if (pressedEnter) {
				...
				FlxG.camera.flash(ClientPrefs.data.flashingLights ? FlxColor.WHITE : 0x4CFFFFFF, 1);
	...
	override function beatHit() {
		...
		if (!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					#if WATERMARKS
					createCoolText(['Imagination Engine by'], 40);
					#else
				...
				case 4:
					#if WATERMARKS
					addMoreText('Rodney', 40);
					addMoreText('Atlas', 40);
					#else
					addMoreText('present');
					#end
				// credTextShit.text += '\npresent...';
