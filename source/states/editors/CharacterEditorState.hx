...
class CharacterEditorState extends MusicBeatState {
	...
	function addCharacterUI() {
		...
		noAntialiasingCheckBox.callback = function() {
			...
			if (!noAntialiasingCheckBox.checked && ClientPrefs.data.antialiasing) char.antialiasing = true;
	...
	function addAnimationsUI() {
		...
		var addUpdateButton:FlxButton = new FlxButton(70, animationIndicesInputText.y + 30, "Add/Update", function() {
			...
			var lastAnim:String = '';
			if (char.animationsArray[curAnim] != null) lastAnim = char.animationsArray[curAnim].anim;

			var lastOffsets:Array<Int> = [0, 0];
			for (anim in char.animationsArray) {
				if (animationInputText.text == anim.anim) {
					lastOffsets = anim.offsets;
					if (char.animation.getByName(animationInputText.text) != null) char.animation.remove(animationInputText.text);
			...
			if (indices != null && indices.length > 0) char.animation.addByIndices(newAnim.anim, newAnim.name, newAnim.indices, "", newAnim.fps, newAnim.loop);
			...
			if (lastAnim == animationInputText.text) {
				...
				if (leAnim != null && leAnim.frames.length > 0) char.playAnim(lastAnim, true);
				else {
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if (id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			if (sender == scaleStepper) {
				...
				updatePointerPos();
			} else if (sender == singDurationStepper) char.singDuration = singDurationStepper.value; //ermm you forgot this??
	...
	function reloadCharacterImage() {
		...
		if (Paths.fileExists('images/' + char.imageFile + '/Animation.json', TEXT)) char.frames = AtlasFrameMaker.construct(char.imageFile);
		...
		if (char.animationsArray != null && char.animationsArray.length > 0) {
			for (anim in char.animationsArray) {
				...
				if (animIndices != null && animIndices.length > 0) char.animation.addByIndices(animAnim, animName, animIndices, "", animFps, animLoop);
	...
	function updatePointerPos() {
		...
		if (!char.isPlayer) x += 150 + char.cameraPosition[0];
	...
	function reloadGhost() {
		...
		for (anim in char.animationsArray) {
			...
			var animLoop:Bool = !!anim.loop; //Bruh
			var animIndices:Array<Int> = anim.indices;
			if (animIndices != null && animIndices.length > 0) ghostChar.animation.addByIndices(animAnim, animName, animIndices, "", animFps, animLoop);
