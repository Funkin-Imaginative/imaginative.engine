...
class Paths {
	...
	inline static public function voices(song:String, ?isBF:Bool = null, ?suffix:String = null):Any {
		var sepStr:String = '';
		if (isBF != null) sepStr = isBF ? 'play' : 'oppo';
	...
	static public function getTextFromFile(key:String, ?ignoreMods:Bool = false):String {
		#if sys
		#if MODS_ALLOWED if (!ignoreMods && FileSystem.exists(modFolders(key))) return File.getContent(modFolders(key)); #end
	...
	static public function getAtlas(key:String, ?library:String):FlxAtlasFrames {
		#if MODS_ALLOWED if (FileSystem.exists(modsXml(key)) || OpenFlAssets.exists(file('images/$key.xml', library), TEXT))
		#else if (OpenFlAssets.exists(file('images/$key.xml', library))) #end return getSparrowAtlas(key, library);
	...
	inline static public function getSparrowAtlas(key:String, ?library:String):FlxAtlasFrames {
		...
		#else return FlxAtlasFrames.fromSparrow(image(key, library), file('images/$key.xml', library)); #end
	}
	...
	inline static public function getPackerAtlas(key:String, ?library:String):FlxAtlasFrames {
		...
		#else return FlxAtlasFrames.fromSpriteSheetPacker(image(key, library), file('images/$key.txt', library)); #end
	}
	...
	public static function returnSound(path:String, key:String, ?library:String) {
		...
		// trace(gottenPath);
		if (!currentTrackedSounds.exists(gottenPath)) #if MODS_ALLOWED currentTrackedSounds.set(gottenPath, Sound.fromFile('./' + gottenPath));
		#else {
			var folder:String = '';
	...
	static public function getModDirectories():Array<String> {
		...
		if (FileSystem.exists(modsFolder)) {
			for (folder in FileSystem.readDirectory(modsFolder)) {
				...
				if (sys.FileSystem.isDirectory(path) && !ignoreModFolders.contains(folder) && !list.contains(folder)) list.push(folder);
			}
		}
		return list;
	}
