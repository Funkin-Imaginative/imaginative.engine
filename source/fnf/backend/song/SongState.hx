...
class SongState extends FlxTransitionableState implements ISong implements ISongLoadable<SongState>{
	public static var direct:SongState;
	...
	public function stepHit(curStep:Int):Void {
		for (member in members) if (member is ISong) cast(member, ISong).stepHit(curStep);
	...
	public function beatHit(curBeat:Int):Void {
		for (member in members) if (member is ISong) cast(member, ISong).beatHit(curBeat);
		call('beatHit', [curBeat]);
		if (curBeat & 4 == 0) measureHit(curMeasure);
	}

	public function measureHit(curMeasure:Int):Void {
		for (member in members) if (member is ISong) cast(member, ISong).measureHit(curMeasure);
	...
	override public function destroy() {
		...
		try {if (PlayField.direct.state == this) PlayField.direct.state = null;} catch(e:haxe.Exception) {}
		direct = null;
		super.destroy();
	}
}