...
class FunkinCamera extends FlxCamera {
	 ...
	 * only lines 50 and 51 are changed... I think those are the lines anyway
	 */
	override function updateFollow():Void {
		...
		if (deadzone == null) {
			...
			if (style == SCREEN_BY_SCREEN) {
				if (targetX >= (scroll.x + width)) _scrollTarget.x += width;
				else if (targetX < scroll.x) _scrollTarget.x -= width;

				if (targetY >= (scroll.y + height)) _scrollTarget.y += height;
				else if (targetY < scroll.y) _scrollTarget.y -= height;
			} else {
			...
			if (followLerp >= 60 / FlxG.updateFramerate) scroll.copyFrom(_scrollTarget); // no easing
			else { // THIS THE PART THAT ACTUALLY MATTERS LOL
