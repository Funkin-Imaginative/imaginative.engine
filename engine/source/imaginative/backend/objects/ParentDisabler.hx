 ...
 * - Sounds
 *
 * To use, add `add(new ParentDisabler());` after `super.create();` in your `create` function.
 * @author @CodenameCrew
 */
@SuppressWarnings('checkstyle:CommentedOutCode') // no camera support atm
class ParentDisabler extends FlxBasic {
	...
	override public function new(replaceUponDestroy:Bool = false, restoreUponDestroy:Bool = true) {
		super();
